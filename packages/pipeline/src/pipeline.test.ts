...
const GOAL: StrategyGoal = {
  ...
  horizon: "12 months",
  chains: ["ethereum"],
  targetYield: 800,  // 8% target APY
};

function mockInference(jsonResp: object): InferenceClient {
  return {
    infer: mock(
      async (): Promise<Result<SealedInferenceResult>> =>
        ok({
...
describe("Pipeline Pipeline Components", () => {
  it("Researcher generates LogicNodeConfigs", async () => {
    ...
    const result = await researcher.run({
      ...
      actionSchemas: [{ type: "action.mock" } as any],
    });
  ...
  it("Strategist generates CandidateWorkflows", async () => {
    ...
    const resOutput = {
      ...
      filteredOut: [],
      rebalanceFrequency: "0 12 * * *",
      evidence: {} as any,
    };
    const result = await strategist.run({
      goal: GOAL,
      researcherOutput: resOutput,
      priorVersions: [],
      actionSchemas: [],
    });
    expect(result.ok).toBe(true);
  ...
  it("Strategist salvages JS-style stringified workflow parts", async () => {
        ...
        candidates: [
          {
            ...
            nodes:
              '[{ id: "check-balance", type: "web3/check-balance", label: "Check Balance", config: { network: "1", address: "0xYOUR_WALLET_ADDRESS" } }, { id: "notify", type: "discord/send-message", label: "Notify", config: { discordMessage: "ready" } }]',
            edges:
              '[{ source: "trigger", target: "check-balance" }, { source: "check-balance", target: "notify" }]',
    ...
    expect(result.value.candidates[0]?.nodes.map((node) => node.id)).toEqual([
      "check-balance",
      "notify",
    ]);
  ...
  it("Strategist retries with a repair pass before placeholder fallback", async () => {
    ...
    expect((result.value.evidence.output as any).recoveryStagesUsed).toEqual([
      "repair",
    ]);
    expect((result.value.evidence.output as any).usedFallback).toBe(false);
  });
  ...
  it("Compiler generates WorkflowSpecs", async () => {
    const hub = {
      listActionSchemas: async () =>
        ok([{ type: "aave-v3/supply", label: "test" }]),
    };
    ...
    const candidate: CandidateWorkflow = {
      id: "A",
      nodes: [{ id: "aave-v3", type: "action", config: {} }],
      edges: [{ source: "trigger", target: "aave-v3" }],
      hypothesis: "",
      confidence: 0.9,
    };
  ...
  it("RiskValidator passes valid node-edge setups", () => {
    ...
    const spec = {
      nodes: [{ id: "node1", type: "aave-v3/supply", config: {} }],
      edges: [{ source: "trigger", target: "node1" }],
      name: "test",
      description: "test",
    ...
    const candidate: CandidateWorkflow = {
      id: "A",
      nodes: [
        { id: "aave-v3", type: "action", config: { maxSlippage: "0.1%" } },
      ],
      edges: [],
      hypothesis: "",
      confidence: 1,
