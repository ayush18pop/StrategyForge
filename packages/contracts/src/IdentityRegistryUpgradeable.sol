    ...
    EIP712Upgradeable
{
    ...
    bytes32 private constant RESERVED_AGENT_WALLET_KEY_HASH =
    ...
    function register() external returns (uint256 agentId) {
        ...
        agentId = _registerAgent("", emptyMetadata);
    }
    ...
    {
        MetadataEntry[] memory emptyMetadata = new MetadataEntry[](0);
        agentId = _registerAgent(agentURI, emptyMetadata);
    }
    ...
    function register(
        ...
        agentId = _registerAgent(agentURI, metadata);
    }
    ...
    /// @notice Compatibility alias for the existing StrategyForge server layer.
    function updateAgent(uint256 agentId, string calldata agentURI) external {
        _requireAuthorized(agentId);
    ...
    function _getIdentityRegistryStorage()
        private
        pure
        returns (IdentityRegistryStorage storage $)
    {
        assembly {
