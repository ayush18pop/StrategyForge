...
interface IIdentityRegistryAuthorization {
...
contract ReputationRegistryUpgradeable is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    struct ReputationRegistryStorage {
        ...
        mapping(uint256 => mapping(address => mapping(uint64 => mapping(address => uint64))))
            _responseCount;
        mapping(uint256 => mapping(address => mapping(uint64 => address[])))
            _responders;
        mapping(uint256 => mapping(address => mapping(uint64 => mapping(address => bool))))
            _responderExists;
    ...
    function giveFeedback(
        ...
        require(
            value >= -MAX_ABS_VALUE && value <= MAX_ABS_VALUE,
            "value too large"
        );
        require(
            !IIdentityRegistryAuthorization(_identityRegistry)
    ...
    function revokeFeedback(uint256 agentId, uint64 feedbackIndex) external {
        ...
        require(
            !$._feedback[agentId][msg.sender][feedbackIndex].isRevoked,
            "Already revoked"
        );
    ...
    function appendResponse(
        ...
        if (
            !$._responderExists[agentId][clientAddress][feedbackIndex][
                msg.sender
            ]
        ) {
            $._responders[agentId][clientAddress][feedbackIndex].push(
                msg.sender
            );
            $._responderExists[agentId][clientAddress][feedbackIndex][
                msg.sender
        ...
        )
    {
        ...
        for (uint256 i = 0; i < clientList.length; i++) {
            ...
            for (uint64 j = 1; j <= lastIdx; j++) {
                ...
                if (
                    emptyHash != tag1Hash &&
                    tag1Hash != keccak256(bytes(fb.tag1))
                ) continue;
                if (
                    emptyHash != tag2Hash &&
                    tag2Hash != keccak256(bytes(fb.tag2))
                ) continue;
        ...
        summaryValue = int128(
            avgWad / int256(10 ** uint256(18 - summaryValueDecimals))
        );
    }
    ...
    function _countResponses(
        ...
        if (responders.length == 0) {
            address[] memory allResponders = $._responders[agentId][
                clientAddress
            ][feedbackIndex];
            for (uint256 k = 0; k < allResponders.length; k++) {
                count += $._responseCount[agentId][clientAddress][
                    feedbackIndex
                ][allResponders[k]];
            }
        } else {
    ...
    function _getReputationRegistryStorage()
        private
        pure
        returns (ReputationRegistryStorage storage $)
    {
        assembly {
