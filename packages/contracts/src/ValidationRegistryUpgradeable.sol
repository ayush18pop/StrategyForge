...
interface IIdentityRegistryApproval {
...
contract ValidationRegistryUpgradeable is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function validationRequest(
        ...
        require(
            $.validations[requestHash].validatorAddress == address(0),
            "exists"
        );

        IIdentityRegistryApproval registry = IIdentityRegistryApproval(
            _identityRegistry
        );
    ...
    function _getValidationRegistryStorage()
        private
        pure
        returns (ValidationRegistryStorage storage $)
    {
        assembly {
