...
function AttestationId({ id }: { id?: string }) {
  ...
  return (
    <button
      ...
      aria-label={`Copy attestation ID: ${id}`}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: "8px",
    ...
    >
      ...
      <span
        style={{
          fontFamily: "var(--font-mono)",
          fontSize: "10px",
          color: copied ? "var(--accent-verify)" : "rgba(0,229,200,0.4)",
...
function colorizeJson(json: string): string {
  return json
    ...
    .replace(
      /"([^"]+)":/g,
      '<span style="color:rgba(255,255,255,0.65)">"$1"</span>:',
    )
    .replace(
      /: "([^"]+)"/g,
      ': <span style="color:var(--accent-verify)">"$1"</span>',
    )
    .replace(
      /: (-?\d+\.?\d*)/g,
      ': <span style="color:var(--attest-500)">$1</span>',
    )
    .replace(
      /: (true|false|null)/g,
      ': <span style="color:var(--ok-500)">$1</span>',
    );
}
    ...
    >
      ...
      <div style={{ padding: "20px 24px 16px" }}>
        ...
        <button
          ...
          onMouseOver={(e) =>
            (e.currentTarget.style.color = "var(--accent-verify)")
          }
          onMouseOut={(e) =>
